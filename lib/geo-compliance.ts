...
const geoApiKeys = {
  IPGEO_API_KEY: process.env.NEXT_PUBLIC_IPGEO_API_KEY,
  IPREGISTRY_KEY: process.env.NEXT_PUBLIC_IPREGISTRY_KEY,
  BIGDATACLOUD_KEY: process.env.NEXT_PUBLIC_BIGDATACLOUD_KEY,
};
...
export const getCurrentLocation = async (): Promise<string[]> => {
  const fetchLocation = async (url: string, getCountry: (data: any) => string[]) => {
    try {
...
export const checkUserCountryRestriction = (
  ...
  for (const location of userLocation) {
    for (const restricted of restrictedLocations) {
      if (
        restricted.toLowerCase().includes(location.toLowerCase())
      ) {
