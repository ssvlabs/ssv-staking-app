    ...
    <Dialog isOpen={modal.isOpen}>
      <DialogContent className="w-[560px] max-w-[calc(100%-32px)] gap-5 p-8">
        ...
        <div className="flex w-full flex-col gap-2">
          {txs.map((tx, i) => (
            <TransactionCard key={i} tx={tx} onRetry={() => send({ type: "retry" })} />
          ))}
        </div>
...
// Keying on `_id` fully unmounts/remounts `BatchTransactionModalContent` each
// time the modal signal fires. This means `useMachine` inside the content
// component is created fresh for every new batch and destroyed when the modal
// closes — no stale state, no manual resets, and zero cost while the modal is hidden.
